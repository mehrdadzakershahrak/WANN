...
def viewInd(ind, env_config):
    ...
    # Create Graph
    nIn = env_config.input_size + 1  # bias
    nOut = env_config.output_size
    G, layer = ind2graph(wMat, nIn, nOut)
    pos = getNodeCoord(G, layer, env_config)
...
def getNodeCoord(G, layer, env_config):
    # env = games[taskName]
    # Calculate positions of input and output
    nIn = env_config.input_size + 1
    nOut = env_config.output_size
    nNode = len(G.nodes)

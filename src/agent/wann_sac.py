...
from agent.wann_off_policy_algorithm import OffPolicyAlgorithm
...
class SAC(OffPolicyAlgorithm):
    ...
    def __init__(
        ...
        self.use_wann = use_wann
        self.wann_wVec = wann_wVec
        self.wann_aVec = wann_aVec
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            replay_data = self.mini_replay_buffer.sample_wann(batch_size, env=self._vec_normalize_env)

            # We need to sample because `log_std` may have changed between two gradient steps
            if self.use_sde:
                self.actor.reset_noise()

            # TODO: flatten if not mlp

            if self.use_wann:
                obs_batch = replay_data.observations
            else:
                obs_batch = replay_data.wann_observations
            ...
            with th.no_grad():
                if self.use_wann:
                    next_obs_batch = replay_data.wann_next_observations
                else:
                ...
                targets = th.cat(self.critic_target(next_obs_batch, next_actions), dim=1)
            ...
            q_values_pi = th.cat(self.critic.forward(obs_batch, actions_pi), dim=1)
